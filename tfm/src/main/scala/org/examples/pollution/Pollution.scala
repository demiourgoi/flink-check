  ...
  def main(args: Array[String]) {
    ...
    accumulateWindow.keyBy(1)
      .mapWithState((in: (Int, Int), alarm: Option[Boolean]) =>
        alarm match {
          case Some(c) => if(c && in._1 == 0) ( "Alarma desactivada en sensor " + in._2, Some(false) ) else if (!c && in._1 == 2) ("Alarma activada en sensor " + in._2, Some(true) ) else ("Sin cambios en sensor " + in._2, Some(c))
          case None => if(in._1 == 2) ("Alarma activada en sensor " + in._2, Some(true) ) else ("Sin cambios en sensor " + in._2, Some(false))
        }).print()
